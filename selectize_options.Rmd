...
ui <- fluidPage(
  ...
  selectizeInput(
    ...
    multiple = TRUE, 
    options = list(
      plugins = list("drag_drop", "optgroup_columns")
    )
  )
)
...
## DRY?
So far so good, right? Right. You might then ask me, but Mwavu great R programmers like [Hadley Wickham](http://hadley.nz/) emphasize the DRY principle (Don't Repeat Yourself). By setting those options on every `selectizeInput` is kinda wet... Yunno, not dry. (God, ik I'm so funny. Ha. Ha.) How do you avoid that? Simple. Use a function.
```{r}
...
I have a small sample app below:
```{r}
library(shiny)
library(shinydashboard)
